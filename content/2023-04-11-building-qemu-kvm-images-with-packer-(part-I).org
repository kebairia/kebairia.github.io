...
- Packer installed on you system
...
Let's install qemu's plugin. Add this to =main.pkr.hcl=
...
- =source= blocks contain configuration for builder plugins, including hardware and resource allocation settings such as type of hardware, RAM and CPU requirements.
  Each source block has a unique name that can be referenced in the =build= block.

- =provisioner= blocks contain your provisionners, like shell scripts, Ansible playbook..etc.
...
In the =provisioner= block, we used a shell script called =configs.sh=.
** Kickstart file
