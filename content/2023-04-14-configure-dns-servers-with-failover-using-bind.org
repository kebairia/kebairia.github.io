...
#+PROPERTY: header-args:sh :dir /ssh:admin1: :results output :exports both 
#+begin_date
Published on {{{date}}}
#+end_date
...
Because DNS server are called nameserver in Linux terminology.
...
Let's make sure that the service in enabled at startup.
#+begin_src sh :results none
...
This will help us seperate the the parts of the configuraton that are somehow static from those that can be changed overtime.\\
...
For the backup DNS server, it's basicaly the same configuration, with just some modifications that indicate that this server is related to the idea that this server must behave as 
a secondary DNS server.
The main configuration for the secondary DNS server
...
For client, we just need to change our main DNS server to the IP addresses of our new local DNS server.
